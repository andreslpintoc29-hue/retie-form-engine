...
export default function RSptPage() {
  ...
  useEffect(() => {
    async function loadEvidencias() {
      ...
      try {
        ...
        console.error('Error loading evidences R_SPT:', err);
      }
    }
  ...
  const handleSaveEvidencias = useCallback(async (updated: any[]) => {
    ...
    try {
      ...
      console.error('Error saving evidences R_SPT:', err);
    }
  }, [baseInspectionId]);
        ...
        {SCHEMA.secciones.map((sec: any, sIdx: number) => {
          ...
          if (sec.tipo === 'informativo_con_imagen') {
            return (
              <div
                key={sIdx}
                className="bg-slate-800 rounded-xl border border-slate-700 p-6 flex flex-col lg:flex-row gap-6 items-center"
              >
                <div className="flex-1 space-y-4">
                  ...
                  {sec.campos?.map((campo: any) => (
                    <div
                      key={campo.id}
                      className="bg-slate-900/40 rounded-lg p-3 border border-slate-700 shadow-sm"
                    >
                ...
                <div className="flex-1 flex flex-col justify-center items-center p-6 bg-slate-950 rounded-xl border border-slate-700 shadow-lg min-h-[280px] w-full">
                  <span className="text-blue-400 text-xs font-bold uppercase tracking-wider mb-4">
                ...
                <div className="divide-y divide-slate-700">
                  {sec.preguntas?.map((p: any) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className="p-4 md:p-5 flex flex-col gap-4 hover:bg-slate-800/30 transition-colors"
                      >
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                          <div className="flex-1 pr-4">
                            <p className="text-sm font-medium text-slate-350 whitespace-pre-wrap leading-relaxed">
                              <span className="inline-block px-1.5 py-0.5 bg-slate-900 rounded text-blue-400 text-xs font-bold mr-2">
                                {p.numero}
                              </span>
                              {p.pregunta}
                            </p>
