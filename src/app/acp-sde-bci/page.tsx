...
const SCHEMA = RETIE_ACP_SDE_BCI_SCHEMA as any;
const OPCIONES_V1 = ['SI', 'NO', 'N/A'];
const OPCIONES_CIERRE = ['SI', 'NO'];

function normalizeAcpSdeBciAnswersForPdf(answers: Record<string, any>, schema: any) {
...
export default function AcpSdeBciPage() {
  const [inspectionId] = useState(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const id = params.get('id') || 'RETIE-DEMO-2026';
      return `${id}_ACPSDEBCI`;
    }
    return 'RETIE-DEMO-2026_ACPSDEBCI';
  });
  ...
  useEffect(() => {
    async function loadEvidencias() {
      ...
      } catch (err) { console.error('Error loading evidences ACP:', err); }
    }
    loadEvidencias();
  }, [baseInspectionId]);
  ...
  const handleSaveEvidencias = async (updated: any[]) => {
    ...
    } catch (err) { console.error('Error saving evidences ACP:', err); }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col">
      ...
      <header className="bg-slate-900/95 border-b border-slate-700 sticky top-0 z-30 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-violet-300 leading-tight">⚡ {SCHEMA.titulo}</h1>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2">
              ...
              <span className="text-xs text-violet-300 font-mono">{progress}%</span>
            </div>
            {lastSaved && <span className="text-[10px] text-slate-500">💾 {lastSaved}</span>}
            <button
              onClick={async () => {
                const answers = (formEngine as any).store?.answers || {};
                const normalizedAnswers = normalizeAcpSdeBciAnswersForPdf(answers, SCHEMA);
                const pdf = await pdfEngine.generateReport({
                  mode: 'single-form',
                  targetForm: 'ACPSDEBCI',
                  formACPSDEBCI: normalizedAnswers,
                  inspection: {
                    inspectionId,
                    inspectionCode: inspectionId,
                    status: 'in_progress',
                    siteName: answers['site_name'] || 'ACP-SDE-BCI',
                    siteAddress: answers['site_address'] || 'Dirección',
                    inspectionDate: answers['fecha_inspeccion'] || new Date().toISOString(),
      ...
      <main className="max-w-5xl mx-auto w-full px-4 py-6 space-y-5">
        ...
        {/* INSTRUCCIONES */}
        {SCHEMA.instruccion_excel && (
          <div className="bg-violet-950/30 border border-violet-800/40 rounded-xl px-4 py-2.5">
            <p className="text-xs text-violet-300 italic">📌 {SCHEMA.instruccion_excel}</p>
          </div>
        )}

        {/* ENCABEZADOS / IMÁGENES */}
        {SCHEMA.imagenes_embebidas?.length > 0 && (
          <section className="bg-slate-800 border border-slate-700 rounded-xl p-4">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">🖼️ Encabezados del Formato</h2>
            <div className="flex flex-wrap gap-3">
              {SCHEMA.imagenes_embebidas.map((img: any) => (
                <div key={img.id} className="bg-slate-700 rounded-lg px-4 py-2.5 text-xs text-slate-300 border border-slate-600">
                  <span className="text-violet-400 font-mono mr-2">📄</span>{img.descripcion || img.file}
                </div>
              ))}
            </div>
          </section>
            ...
            {SCHEMA.campos_contexto?.map((c: any) => (
              <div key={c.id}>
                ...
                <input
                  ...
                  className="w-full bg-slate-700 border border-slate-600 text-white text-sm rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-violet-500"
                />
              </div>
            ))}
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 flex-wrap">
          {secciones.map((sec: any, i: number) => {
            ...
            const total2 = sec.preguntas?.length || 0;
            return (
              <button key={sec.id} onClick={() => setActiveSeccion(i)}
                className={`shrink-0 px-3 py-2 rounded-lg text-xs font-semibold border transition-all text-left ${
                  activeSeccion === i ? 'bg-violet-700 border-violet-600 text-white' : 'bg-slate-800 border-slate-700 text-slate-400 hover:border-violet-600 hover:text-violet-300'}`}>
                <span className="block">{sec.titulo}</span>
                <span className="text-[9px] opacity-70">{answered}/{total2}</span>
              </button>
            );
          })}
        </div>
        ...
        {seccionActual && (
          <section className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
            <div className="bg-gradient-to-r from-violet-950/60 to-slate-800 px-5 py-3 border-b border-slate-700 flex items-center justify-between">
              <h3 className="text-sm font-bold text-violet-200">{seccionActual.titulo}</h3>
                  ...
                  <div key={p.id} className={`px-5 py-3 transition-colors ${isNO ? 'bg-red-950/20' : 'hover:bg-slate-750'}`}>
                    <div className="grid gap-3 items-start" style={{ gridTemplateColumns: '2rem 1fr 6rem 7rem 5rem' }}>
                      ...
                      <div>
                        ...
                        {p.valor_excel && (
                          <span className="text-[9px] text-slate-500 mt-0.5 block">Excel: {p.valor_excel}</span>
                        )}
                        {isNO && (
                          <>
                            <span className="text-[10px] text-red-400 mt-1 block">⚠️ No Conformidad</span>
                            <textarea rows={2} placeholder="Observación NC..."
                      ...
                      <span className="text-[10px] text-slate-500 text-center mt-0.5 break-words">{p.norma}</span>
        ...
        <section className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          ...
          <textarea rows={4} value={getValue('observaciones') ?? ''}
            ...
            className="w-full bg-slate-700 border border-slate-600 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-violet-500 resize-none" />
        </section>

        {/* PROGRESO */}
        <section className="bg-gradient-to-br from-violet-950/40 to-slate-800 border border-violet-800/30 rounded-xl p-5">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-bold text-violet-200">📊 Progreso de Inspección</h3>
            <span className="text-lg font-bold text-violet-300 font-mono">{progress}%</span>
          </div>
          <div className="w-full bg-slate-700 rounded-full h-2.5">
            <div className="bg-gradient-to-r from-violet-600 to-purple-400 h-2.5 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
          </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-8">
          <div className="bg-slate-800 p-6 rounded-lg max-w-xl w-full border border-slate-700">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">📄 PDF Generado — ACP-SDE-BCI</h3>
              <button onClick={() => setShowPdfPreview(false)} className="text-slate-400 hover:text-white">✕</button>
            </div>
            <p className="text-slate-400 mb-4">
              Lista de Chequeo ACP-SDE-BCI lista para descargar.
            </p>
