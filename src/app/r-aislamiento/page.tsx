...
export default function RAislamientoPage() {
  ...
  const baseInspectionId = inspectionId.replace('_R_AISLAMIENTO', '');
  ...
  // Load evidences from base expediente
  useEffect(() => {
    async function loadEvidencias() {
      ...
      try {
        ...
        if (cached?.evidencias) {
          setEvidencias(cached.evidencias);
        }
      } catch (err) {
        console.error('Error loading evidences R_AISLAMIENTO:', err);
      }
    }
  ...
  const handleSaveEvidencias = useCallback(async (updated: any[]) => {
    ...
    try {
      const cached = await offlineEngine.getExpediente(baseInspectionId) || {};
      await offlineEngine.saveExpediente(baseInspectionId, {
        ...cached,
        evidencias: updated,
      });
    } catch (err) {
      console.error('Error saving evidences R_AISLAMIENTO:', err);
    }
  }, [baseInspectionId]);
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-white text-xl animate-pulse">
          Cargando formulario de aislamiento...
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-bold text-blue-400 leading-none">
                  {SCHEMA.titulo}
                </h1>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="hidden md:flex text-[10px] text-slate-500 mr-2">
        ...
        <div className="bg-gradient-to-r from-blue-900/20 via-slate-800 to-slate-800 rounded-xl border border-slate-700 p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              El valor de resistencia de aislamiento debe medirse en todos los conductores activos con respecto a tierra.
            </p>
          </div>
          ...
          <div className="shrink-0 max-w-xs w-full">
            ...
            <p className="text-[9px] text-slate-500 mt-1 text-center font-mono">
              📷 {SCHEMA.recursos_visuales_hoja[0].file}
            </p>
          </div>
        </div>
        ...
        {SCHEMA.secciones.map((sec: any, sIdx: number) => {
            ...
            return (
              <div key={sIdx} className="space-y-3">
                ...
                <EvidenceUploader
                  ...
                  formId="R_AISLAMIENTO"
                  questionId={sec.id || `section_${sIdx}`}
                  caption={sec.titulo || 'Sección técnica R. Aislamiento'}
          ...
          if (sec.tipo === 'informativo_con_imagen') {
            ...
            return (
              <div
                key={sIdx}
                className="bg-slate-800 rounded-xl border border-slate-700 p-6 flex flex-col lg:flex-row gap-6 items-center shadow-sm"
              >
                <div className="flex-1 space-y-4">
                  ...
                  {sec.campos?.map((campo: any) => (
                    <div
                      key={campo.id}
                      className="bg-slate-900/40 rounded-lg p-4 border border-slate-700 shadow-inner"
                    >
          ...
          if (sec.tipo === 'textarea') {
            ...
            return (
              ...
              >
                <h3 className="font-bold text-blue-400 text-sm uppercase tracking-wide mb-3">
                  {sec.titulo}
                </h3>
                ...
                <div className="mt-3">
                  <EvidenceUploader
                    ...
                    formId="R_AISLAMIENTO"
                    questionId={fieldId}
                    caption={sec.titulo || 'Observaciones R. Aislamiento'}
                    evidencias={evidencias}
                    onSaveEvidencias={handleSaveEvidencias}
                  />
                </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl p-6 max-w-sm w-full border border-slate-700">
            <h3 className="text-lg font-bold text-white mb-2">
              ¿Finalizar módulo R.AISLAMIENTO?
            </h3>
