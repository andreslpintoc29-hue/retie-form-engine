...
const SCHEMA = RETIE_UFIYC_SCHEMA as any;
const OPCIONES_V1 = ['SI', 'NO', 'N/A'];
const OPCIONES_CIERRE = ['SI', 'NO'];

function normalizeUfiycAnswersForPdf(answers: Record<string, any>, schema: any) {
  ...
  schema.secciones?.forEach((sec: any) => {
    sec.preguntas?.forEach((p: any) => {
      ...
      // Accept multiple possible storage shapes
...
export default function UfiycPage() {
  ...
  useEffect(() => {
    async function loadEvidencias() {
      ...
      } catch (err) { console.error('Error loading evidences UFIYC:', err); }
    }
    loadEvidencias();
  }, [baseInspectionId]);
  ...
  const handleSaveEvidencias = async (updated: any[]) => {
    ...
    } catch (err) { console.error('Error saving evidences UFIYC:', err); }
  };
      ...
      <header className="bg-slate-900/95 border-b border-slate-700 sticky top-0 z-30 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-orange-300 leading-tight">🏭 LISTA DE CHEQUEO USO FINAL INDUSTRIAL Y COMERCIAL</h1>
              <p className="text-[10px] text-slate-400">F-GI-06 · v2 · {SCHEMA.proceso}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2">
              ...
              <span className="text-xs text-orange-300 font-mono">{progress}%</span>
            </div>
            {lastSaved && <span className="text-[10px] text-slate-500">💾 {lastSaved}</span>}
            <button
              onClick={async () => {
                const answers = (formEngine as any).store?.answers || {};
                const normalizedAnswers = normalizeUfiycAnswersForPdf(answers, SCHEMA);
                console.debug('UF I Y C PDF normalized answers:', {
                  keys: Object.keys(normalizedAnswers).length,
                  sample: Object.fromEntries(Object.entries(normalizedAnswers).slice(0, 20)),
                });
                const pdf = await pdfEngine.generateReport({
                  mode: 'single-form',
                  targetForm: 'UFIYC',
                  formUFIYC: normalizedAnswers,
                  inspection: {
                    inspectionId,
                    inspectionCode: inspectionId,
                    status: 'in_progress',
                    siteName: normalizedAnswers['site_name'] || 'Uso Final Industrial y Comercial',
      ...
      <main className="max-w-5xl mx-auto w-full px-4 py-6 space-y-5">
            ...
            {SCHEMA.campos_contexto?.map((c: any) => (
              <div key={c.id}>
                ...
                <input
                  ...
                  className="w-full bg-slate-700 border border-slate-600 text-white text-sm rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
              </div>
            ))}
          </div>
            ...
            return (
              <button key={sec.id} onClick={() => setActiveSeccion(i)}
                className={`shrink-0 px-3 py-2 rounded-lg text-xs font-semibold border transition-all text-left ${
                  activeSeccion === i ? 'bg-orange-700 border-orange-600 text-white' : 'bg-slate-800 border-slate-700 text-slate-400 hover:border-orange-700 hover:text-orange-300'}`}>
        ...
        {seccionActual && (
          <section className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
            <div className="bg-gradient-to-r from-orange-950/60 to-slate-800 px-5 py-3 border-b border-slate-700 flex items-center justify-between">
              <h3 className="text-sm font-bold text-orange-200">{seccionActual.titulo}</h3>
        ...
        <section className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          ...
          <textarea rows={4} value={getValue('observaciones') ?? ''}
            ...
            className="w-full bg-slate-700 border border-slate-600 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none" />
        </section>

        <section className="bg-gradient-to-br from-orange-950/40 to-slate-800 border border-orange-800/30 rounded-xl p-5">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-bold text-orange-200">📊 Progreso</h3>
            <span className="text-lg font-bold text-orange-300 font-mono">{progress}%</span>
          </div>
          <div className="w-full bg-slate-700 rounded-full h-2.5">
            <div className="bg-gradient-to-r from-orange-600 to-amber-400 h-2.5 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
          </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-8">
          <div className="bg-slate-800 p-6 rounded-lg max-w-xl w-full border border-slate-700">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">📄 PDF Generado — UF I Y C</h3>
              <button onClick={() => setShowPdfPreview(false)} className="text-slate-400 hover:text-white">✕</button>
            </div>
            <p className="text-slate-400 mb-4">
              Lista de Chequeo Uso Final Industrial y Comercial lista para descargar.
            </p>
