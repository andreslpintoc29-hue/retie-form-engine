...
export default function IluminanciaPage() {
  ...
  const baseInspectionId = inspectionId.replace('_ILUMINANCIA', '');
  ...
  // Sincronizar inspectionId desde query params
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const id = params.get('id') || 'RETIE-DEMO-2026';
  ...
  // Cargar evidencias del expediente base
  useEffect(() => {
    async function loadEvidencias() {
      ...
      try {
        ...
        console.error('Error loading evidences ILUMINANCIA:', err);
      }
    }
  ...
  const handleSaveEvidencias = useCallback(async (updated: any[]) => {
    ...
    try {
      ...
      console.error('Error saving evidences ILUMINANCIA:', err);
    }
  }, [baseInspectionId]);
        ...
        <div className="bg-gradient-to-r from-slate-900 via-slate-900 to-slate-850 rounded-2xl border border-slate-850 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-2xl">
          <div className="space-y-2">
            ...
            <h2 className="text-xl font-extrabold text-white tracking-tight">
              Registro General de Mediciones de Iluminancia
            </h2>
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-950/60 rounded-xl border border-slate-800/80 text-xs text-slate-400 group hover:border-slate-700 transition-colors">
              ...
              <div>
                <p className="font-bold text-slate-300">
                  Cabecera de Página ({activeSec.titulo}):
                </p>

                <p className="text-[10px] font-mono text-slate-500 mt-0.5">
                  {activeSec.recursos_visuales_hoja[0].file}
                </p>
              </div>
            </div>
          )}
            ...
            {activeSec.tabla_valores_medidos && (
              <div className="space-y-3">
                ...
                <EvidenceUploader
                  ...
                  formId="ILUMINANCIA"
                  questionId={`tabla_${activeSec.titulo || activeTab}`}
                  caption={`Evidencias de valores medidos - ${activeSec.titulo || 'Iluminancia'}`}
              ...
              <div className="bg-slate-900/60 rounded-2xl border border-slate-850 p-6 shadow-lg">
                ...
                <div className="mt-3">
                  <EvidenceUploader
                    inspectionId={baseInspectionId}
                    formId="ILUMINANCIA"
                    questionId={activeSec.observaciones.campo.id}
                    caption={`Observaciones técnicas - ${activeSec.titulo || 'Iluminancia'}`}
                    evidencias={evidencias}
                    onSaveEvidencias={handleSaveEvidencias}
                  />
                </div>
