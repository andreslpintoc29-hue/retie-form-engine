// ============================================
// DEMO PAGE - apantallamiento CON UX PROFESIONAL
...
function normalizeApantallamientoAnswersForPdf(answers: Record<string, any>, schema: any) {
...
export default function apantallamientoInspectorPage() {
  const router = useRouter();
  const [inspectionId, setInspectionId] = useState('DEMO-LOCAL_APANTALLAMIENTO');
  const [evidencias, setEvidencias] = useState<any[]>([]);
  
  // Usar ref para mantener referencias actualizadas en callbacks async
  ...
  useEffect(() => {
    ...
    console.log("=== LOAD EVIDENCIAS EFFECT #" + loadCountRef.current + " ===");
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (!id) {
        // Redirect to default demo id if missing
        router.replace('/apantallamiento?id=DEMO-LOCAL');
        setInspectionId('DEMO-LOCAL_APANTALLAMIENTO');
      } else {
        setInspectionId(`${id}_APANTALLAMIENTO`);
      }
    }
  }, [router]);

  // Cargar evidencias asociadas a la inspección base desde el expediente de IndexedDB
  useEffect(() => {
  ...
  const saveEvidencias = async (updatedEvidencias: any[]) => {
    console.log("=== saveEvidencias CALLED ===");
    console.log("updatedEvidencias.length:", updatedEvidencias.length);
    console.log("current evidencias.length:", evidencias.length);
    console.log("evidenciasRef.current.length:", evidenciasRef.current.length);
    setEvidencias(updatedEvidencias);
    evidenciasRef.current = updatedEvidencias;
    const baseId = inspectionId.replace('_APANTALLAMIENTO', '');
    const cached = await offlineEngine.getExpediente(baseId) || {};
    const dataToSave = {
      ...cached,
    ...
    await offlineEngine.saveExpediente(baseId, dataToSave);
  };
  ...
  const renderField = (field: any) => {
    ...
    return (
      <div className="space-y-2 p-4 bg-slate-800/50 rounded-lg border border-slate-700/50">
        ...
        {/* Evidencias / Fotos de la Pregunta */}
        <div className="mt-3 space-y-2 border-t border-slate-700/50 pt-2">
          <label className="block text-xs font-semibold text-slate-400">📸 Evidencias / Fotos de la Pregunta:</label>
          
          {evidencias.filter(e => e.formId === 'APANTALLAMIENTO' && e.questionId === String(field.id)).length > 0 && (
            <div className="grid grid-cols-2 gap-2 mt-1">
              {evidencias.filter(e => e.formId === 'APANTALLAMIENTO' && e.questionId === String(field.id)).map((ev: any) => (
                <div key={ev.id} className="relative bg-slate-900 p-2 rounded border border-slate-700">
                  <img src={ev.dataUrl || ev.foto} alt={ev.fileName} className="w-full h-24 object-cover rounded" />
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h1 className="text-xl font-bold text-blue-400">⚡ APANTALLAMIENTO</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-xs text-slate-400">
                {lastSaved ? `💾 ${lastSaved}` : '—'}
                {!formEngine.isReady && (
                  <span className="text-red-400 ml-2">⏳ Iniciando...</span>
                )}
              </div>

              {/* Actions */}
              <button
                disabled={!formEngine.isReady}
                onClick={async () => {
                  const currentStore = (formEngine as any).store;
                  const answers = currentStore?.answers || formEngine.answers || {};

                  console.log("=== ANSWERS EN UI ===");
                  console.log("formEngine.isReady:", formEngine.isReady);
                  console.log("currentStore exists?:", !!currentStore);
                  console.log("answers keys count:", Object.keys(answers).length);
                  console.log("answers entries:", Object.entries(answers).slice(0, 10));
                  console.log("answers completo:", JSON.parse(JSON.stringify(answers)));

                  console.log("=== ANSWERS EN PDF ===");
                  console.log("Enviando al PDF:", JSON.parse(JSON.stringify(answers)));

                  const exactDbKey = `draft_${inspectionId}`;
                  const answersCount = Object.keys(answers).length;
                  console.log("=== DIAGNÓSTICO DE ALMACENAMIENTO (APANTALLAMIENTO) ===");
                  console.log("- inspectionId:", inspectionId);
                  console.log("- clave exacta usada en IndexedDB:", exactDbKey);
                  console.log("- contenido del draft (answers):", answers);
                  console.log("- número de respuestas encontradas:", answersCount);
                  console.log("- primeras 5 respuestas:", Object.entries(answers).slice(0, 5));
                  console.log("- respuestas con prefijo _obs:", Object.keys(answers).filter(k => k.includes('_obs')));

                  const apantallamientoEvidences = evidencias.filter(e => e.formId === 'APANTALLAMIENTO');
                  
                  console.log("=== LOG 0: EVIDENCIAS REF vs STATE ===");
                  console.log("evidencias.length (state):", evidencias.length);
                  console.log("evidenciasRef.current.length:", evidenciasRef.current.length);
                  console.log("evidencias === evidenciasRef.current:", evidencias === evidenciasRef.current);
                  console.log("EVIDENCIAS USADAS EN PDF:", JSON.stringify(evidencias, null, 2));
                  
                  // Leer directamente desde ref para garantizar latest state
                  const directRead = evidenciasRef.current;
                  console.log("directRead from evidenciasRef:", JSON.stringify(directRead, null, 2));
                  
                  console.log("=== LOG 1: EVIDENCIAS EN UI ===");
                  console.log("Total evidencias:", evidencias.length);
                  console.log("APANTALLAMIENTO evidencias:", apantallamientoEvidences.length);
                  
                  apantallamientoEvidences.forEach((ev, idx) => {
                    console.log(`  Evidencia ${idx + 1}:`);
                    console.log(`    id: ${ev.id}`);
                    console.log(`    formId: ${ev.formId}`);
                    console.log(`    questionId: ${ev.questionId}`);
                    console.log(`    fileName: ${ev.fileName}`);
                    console.log(`    dataUrl exists: ${!!ev.dataUrl}`);
                    console.log(`    dataUrl type: ${ev.dataUrl ? ev.dataUrl.split(';')[0] + '...' : 'N/A'}`);
                    console.log(`    dataUrl length: ${ev.dataUrl ? ev.dataUrl.length : 0}`);
                    console.log(`    dataUrl starts with data:image: ${ev.dataUrl?.startsWith('data:image/') || false}`);
                  });
                  
                  console.log("=== LOG 2: ANSWERS EN UI ===");
                  console.log("answers keys count:", Object.keys(answers).length);
                  console.log("answers sample:", JSON.stringify(Object.entries(answers).slice(0, 5)));
                  
                  console.log("=== LOG 3: ENVIANDO AL PDF ===");
                  console.log("mode: single-form");
                  console.log("targetForm: APANTALLAMIENTO");
                  console.log("formApantallamiento keys:", Object.keys(answers).length);
                  console.log("photoEvidence count:", apantallamientoEvidences.length);
                  console.log("photoEvidence[0]:", JSON.stringify(apantallamientoEvidences[0], null, 2));

                  const normalizedAnswers = normalizeApantallamientoAnswersForPdf(answers, SCHEMA);
                  const pdf = await pdfEngine.generateReport({
                    mode: 'single-form',
                    targetForm: 'APANTALLAMIENTO',
                    formApantallamiento: normalizedAnswers,
                    photoEvidence: apantallamientoEvidences,
                    inspection: {
                      inspectionId: inspectionId,
                      inspectionCode: inspectionId,
                      status: 'in_progress',
                      siteName: answers['site_name'] || 'Apantallamiento Demo',
                  ...
                  console.log("=== LOG 4: PDF GENERADO EXITOSAMENTE ===");
                  console.log("PDF blob size:", pdf.size, "bytes");
                  console.log("PDF blob type:", pdf.type);
                  setPdfBlob(pdf);
