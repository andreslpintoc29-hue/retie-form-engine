// ============================================
// DEMO PAGE - PISCINAS CON UX PROFESIONAL
...
function normalizePiscinasAnswersForPdf(answers: Record<string, any>, schema: any) {
...
export default function PiscinasInspectorPage() {
  ...
  const [inspectionId] = useState(() => {
    ...
    return 'RETIE-DEMO-2026_PISCINAS';
  });
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h1 className="text-xl font-bold text-blue-400">🏊 PISCINAS</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                onClick={async () => {
                  const answers = (formEngine as any).store?.answers || {};
                  const normalizedAnswers = normalizePiscinasAnswersForPdf(answers, SCHEMA);
                  const pdf = await pdfEngine.generateReport({
                    mode: 'single-form',
                    targetForm: 'PISCINAS',
                    formPiscinas: normalizedAnswers,
                    inspection: {
                      inspectionId, inspectionCode: inspectionId, status: 'in_progress',
                      siteName: answers['site_name'] || 'Piscina Demo',
              ...
              <button onClick={handleLimpiar} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm font-medium transition-colors border border-slate-600">Limpiar</button>
              ...
              <button onClick={() => {
                const baseId = inspectionId.replace('_PISCINAS', '');
